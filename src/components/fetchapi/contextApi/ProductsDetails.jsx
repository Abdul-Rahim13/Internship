...
function ProductsDetails() {
    const {data, fetchProductDetails, handleInc, handleDec, count} = useContext(AppContext);
    const {id} = useParams()
  ...
  return (
    ...
    <div className="flex justify-center md:justify-start">
        <img src={data.image} alt={data.title} className="w-60 h-60 object-contain border border-gray-200 rounded-xl shadow"/>
    </div>
        
        <div className="mt-4 md:mt-0">

            <h5 className="item-title font-bold text-3xl text-gray-800">{data.title}</h5>
            ...
            <p className="item-price text-blue-600 text-2xl font-bold mt-2">${data.price}.00</p>
            <p className=' text-gray-700 text-justify max-w-xl text-[15px] mt-2'>{data.description}</p>
